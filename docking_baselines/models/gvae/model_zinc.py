...
import docking_baselines.models.gvae.zinc_grammar as G
...
class MoleculeVAE:
    ...
    def create(self, charset, max_length=MAX_LEN, latent_rep_size=2, weights_file=None, lr=2e-4):
        ...
        self.decoder = Model(
            encoded_input,
            self._buildDecoder(encoded_input, latent_rep_size, max_length, charset_length)
        )
        ...
        self.autoencoder = Model(
            x1,
            self._buildDecoder(z1, latent_rep_size, max_length, charset_length)
        )
        ...
        self.autoencoder.compile(optimizer=optimizers.Adam(lr=lr), loss=vae_loss, metrics=['accuracy'])
